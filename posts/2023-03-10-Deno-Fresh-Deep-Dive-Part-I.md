...
Where does it come from? Well if we delete and then start the the dev server using `deno task start` it comes right back. So it's generated during the development process, namely whenever it restarts thanks to the --watch you see the in task definition:
